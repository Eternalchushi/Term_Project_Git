...
$(function() {
    ...
    */
    ...
    /* 
      计算总价的全选
        1、给计算总价的全选按钮 绑定点击事件，获取选择框的状态(true/false)
        2、把状态给表头的全选框 
        3、把状态给表格中的选择框
    */
    $totalPriceInput.change(function() {
        var checkState = $(this).prop('checked'); // 获取计算总价的全选框的选中状态
    ...
    */
    $tbodyInputs.change(function() { //给表格中单选框绑事件
        ...
        $theadInput.prop('checked', flag); // 把状态赋值给头部的全选框
        $totalPriceInput.prop('checked', flag); // 把状态赋值给计算总价的全选框
        allTotal(); //总计
    ...
    */
    $('.reduce').click(function() { // 给减少绑定事件
        ...
        $(this).prev().val(count); // 把自减的值 赋值给前面的输入框
    ...
    */
    function allTotal() {
        ...
        // 渲染
        $('.toltal').text('总价:' + allPrice.toFixed(2)); // 渲染总价
        $('.number').text('已选择' + selectedCount + '商品'); // 渲染数量
    }
